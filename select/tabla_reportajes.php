<?php
    ...
    if(!isset($_POST['filtro'])){
        ...
        $reportajes = $DB->getReportajesOrden(array($filtro,$orden));
    }
    ...
    <tr data-pagina="cabecera" class="fila active">
        ...
        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
            <p class="encabezado" <?php echo ($filtro == "tipo") ? $resaltar : ''?>>
                Tipo 
                <span data-filtro="tipo" data-orden="<?php echo ($filtro == "tipo") ? $inverso : "DESC"?>" onclick="orden(this)"><?php echo ($filtro == "tipo") ? ($inverso == "ASC") ? $asc_res : $desc_res : $desc ?></span>
            </p>
        </th>
        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
